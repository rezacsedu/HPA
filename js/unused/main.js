...
function viewSavedQuery(resourceID){
	$('.collapsibleQuery').hide();
	$('#'+resourceID).show();
}

function loadQuery(resourceID){
	var queryAPI = window.location.protocol + "//" + window.location.host + '/query/'+resourceID;
	$.ajax({
	    url: queryAPI,
	    type: 'GET',
...
function renderPopUp(json,label,image,resource){
	...
	d3.select(selector).html("" +
		/*	"<select name='downloadLens' id='downloadLens'>" +
			"<option value='turtle'>Download as Turtle</option>" +
			...
			"<option value='tsv'>Download as TSV</option></select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
			"<input type='submit' id='downloadResultLens' class='downloadLensBtn btn btn-primary' value = 'Get Results'><br><br>" +*/
...
function buildIncrementElements(allClasses){
	allClasses.sort(function(a, b) {
		 return a.name.localeCompare(b.name);
	}); 
	d3.select("#classParent").selectAll("option").data(allClasses).enter().append("option").text(function(d) { return d.name; }).attr("value", function(d){ return d.uri;});
	d3.select("#objDomain").selectAll("option").data(allClasses).enter().append("option").text(function(d) { return d.name; }).attr("value", function(d){ return d.uri;});
	d3.select("#objRange").selectAll("option").data(allClasses).enter().append("option").text(function(d) { return d.name; }).attr("value", function(d){ return d.uri;});
	d3.select("#dataDomain").selectAll("option").data(allClasses).enter().append("option").text(function(d) { return d.name; }).attr("value", function(d){ return d.uri;});	
}
...
d3.select('#resMetaSubmitBtn').on('click',  function(){
	...
	if(readCookie('disposition') != null){
		...
		makeSparqlRequest(desiredName, $("#resFormatMeta").val());
	}
	else
