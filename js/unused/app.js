...
var ExplorerApp = Backbone.View.extend({
  ...
  _onCreateExplorer: function() {
	  ...
	  if (outputSelection == 'display') {
		  var queryMap = { 'query': queryDisplayValue,	'output' : 'json' }
	   	  var self = this;
	   	  $.ajax({
	   	    type : "POST",
	   	    url : uriStr + "sparql",
...
function makeSparqlRequest (fileName, outputFormat) {
	var queryDisplayValue = $('textarea[name="query"]').val() + " LIMIT " + $('#limitResults').val();
	if(queryDisplayValue != '' && readCookie('disposition') != null){
		...
		if(outputSelection == "save" && outputFormat != "SMI") {
			var queryMap = {
				...
				'disposition' : 'save',
				'filename' : fileName,
				'folderid' : allQueryParams.folderId
			}
			$.ajax({
				type : "POST",
				url : uriStr + "sparql",
			  ...
			  $.ajax({
		   	    ...
		   	    //	console.log(transformedSequence);
		   	    	if(transformedSequence == ""){
		   	    		...
		   	    		storeFile(transformedSequence, fileName, allQueryParams.folderId);
		   	    	}
		   	    }
		   	  });
		} else {
